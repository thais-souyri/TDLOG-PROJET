...
def planning (firm, nb_packages, nb_articles_package):
    ...
    for post in database.Post.select().where(database.Post.firm_name == firm):
        ...
        for i in range(0,6):
            for t in range (0,nb_team[i]):
                while time_needed_day_team[i] > 0 :
                    if not tools.need_interim(firm) :
                        persons_available = tools.persons_available_post(firm).order_by(fn.Random())
        ...
        for person in data.Person.select().where(database.Person.firm_name == firm):
