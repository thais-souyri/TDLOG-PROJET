    ...
    for post in database.Post.select().where(database.Post.firm_name == firm):
        ...
        for person in persons_available:
            ...
            for p in persons_available_post:
                time_operators_post += p.nb_hour_week

        if time_needed > time_operators_post : #un opérateur ne peut pas travailler plus de 5 jours dans la semaine
            time_interim_post = time_needed - time_operators_post
            nb_interim_post = time_interim_post//7 + 1
            nb_interim.append(nb_interim_post)
        else :
            nb_interim.append(0)

        if time_operators_post > 2*6*data.nb_max_team*7: # 2 équipes sur 6 jours travaillant 7h par jour
            time_needed_night = time_operators_post - 2*6 * data.nb_max_team * 7
            nb_operators_night = time_needed_night//7 + 1
            time_operators_post = 2*6 * data.nb_max_team*7

        nb_operators_needed_post = time_operators_post//7 + 1
